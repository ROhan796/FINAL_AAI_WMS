...
function AdminWashroomDetailContent() {
  ...
  const fetchData = useCallback(async () => {
    if (!deviceId) { setError(true); setLoading(false); return }
    try {
  ...
  if (error || !unit) {
    return (
      <div className="p-6 font-sans">
        <Link
          href={`/admin/terminals/${terminalId}/washrooms`}
          className="text-blue-600 hover:underline flex items-center gap-1.5 text-xs font-bold"
        >
    ...
    <div className="space-y-6 font-sans text-sm text-slate-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <PageHeader
          ...
          subtitle={`Device Code: ${unit.device_id} — Admin Inspection View`}
        />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <DataCard title="Admin Operations">
            <div className="space-y-3">
              ...
              <button
                ...
                className="w-full bg-white border border-slate-300 hover:border-slate-400 text-slate-700 hover:bg-slate-50 py-2.5 rounded-xl text-xs font-semibold flex items-center justify-center gap-2 transition-all active:scale-[0.98] cursor-pointer"
              >
                <Wrench size={14} /> View Incidents
              </button>
            </div>
          </DataCard>
        </div>
...
export default function AdminWashroomDetailPage() {
  return (
    <Suspense fallback={<LoadingSpinner text="Loading washroom detail..." />}>
      <AdminWashroomDetailContent />
    </Suspense>
  )
}
